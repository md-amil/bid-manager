...
import {
  AmazonApiService,
  ReportResponse,
} from 'src/services/amazon/amazon-api.service';
...
export class ReportProcessor extends WorkerHost {
  ...
  async process(job: Job, token?: string): Promise<any> {
    ...
    if (reportResponse.status !== 'COMPLETED')
      return Promise.reject({
        ...
        type: reportResponse.configuration.reportTypeId,
      });
  ...
  onError(error: any) {
    if (error instanceof AxiosError)
      return console.error('Worker error:', error.response?.data);
    console.error('Worker error:', error);
  }
  @OnWorkerEvent('completed')
  onComplete(job: Job, result: any) {
    console.log(
      `completed job ${job.id} of type ${job.data.reportTypeId} with reportId ${job.data.reportId}`,
    );
