...
export default class Engine {
  ...
  async buildBundle(campaign: ICampaignDetails): Promise<ICampaignBundle> {
    ...
    const searchTerms = await this.reportService.getSearchTermReport(campaign.campaignId,buildQueryWindow(6));
  ...
  // async daily(campaigns:Campaign[],auth:IAmazonAuth){
  ...
  //   const bundle: Partial<ICampaignBundle> & { campaigns: Campaign[]; budgetUsage: any; searchTerms: ISearchTerm[] } = { campaigns, budgetUsage, searchTerms };
  //   console.log('daily adjustment')
  //    const dailyRules = [
  //     DailyBudgetExhaustionRule,
  //     DailyNegativeKeywordDetectionRule,
  //     DailyASINOutOfStockRule,
  //     DailyPerformanceAlertRule,
  //     DailySpendSpikeRule
  //   ].map(Class => new Class(bundle as ICampaignBundle))
  //   // this.runRuleEngine(manuanCampaignRules, bundle.scopeId)
  // }
