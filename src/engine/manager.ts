...
export default class AdManager extends AutoCampain {
  ...
  analyzeCampaign() {
    if (this.bundle.state == 'PAUSED')
      return console.log('campaign is paused skipping...');
    ...
    if (this._isProfitableSale()) {
      this.recommendations.push(
        new Recommendation({
          ...
          percentageChange: 25,
          reason:
            'Auto campaign producing consistent profitable sales with acceptable ACOS',
          priority: 1,
        }),
      );
    }
    ...
    if (this._goodCVRLowImp()) {
      this.recommendations.push(
        new Recommendation({
          ...
          percentageChange: 10,
          reason:
            'Good Conversion Rate and Limited Impressions Increasing bids by 25% to unlock more inventory',
          priority: 3,
        }),
      );
    }
    if (this._isHighSpendPoorConversion()) {
      this.recommendations.push(
        new Recommendation({
          ...
          percentageChange: 25,
          reason:
            'Lowering bids and budget by 25% and review search terms immediately.',
          priority: 5,
        }),
      );
      this.recommendations.push(
        new Recommendation({
          ...
          percentageChange: 25,
          reason:
            'Lowering  budget and budget by 25% and review search terms immediately.',
          priority: 6,
        }),
      );
    }
