...
export class ViewController {
  constructor(
    ...
    private optimizationLogModel: Model<OptimizationLogDocument>,
    @InjectModel(CampaignReport.name)
  ...
  async getIndex(
    ...
    const allCampaigns = await this.campaignService.getCampaigns(
      session.selectedProfile.profileId,
    );
    const campaigns = allCampaigns.filter(
      (campaign) => campaign.state?.toLowerCase() === 'enabled',
    );
    const recentLogs = await this.bidLogModel
      .find()
      .sort({ createdAt: -1 })
      .limit(10)
      .exec();
  ...
  async getCampaigns(
    ...
    let campaigns = await this.campaignService.getCampaigns(
      session.selectedProfile.profileId,
    );

    if (status) {
      campaigns = campaigns.filter(
        (campaign) => campaign.state.toLowerCase() === status.toLowerCase(),
      );
    }
    if (name) {
    ...
    if (targeting) {
      campaigns = campaigns.filter(
        (campaign) =>
          ...
          campaign.targetingType.toLowerCase() === targeting.toLowerCase(),
      );
    }
    ...
    try {
      ...
      if (reports.length > 0) {
        metrics = {
          ...
          costPerClick:
            reports.length > 0
              ? reports.reduce((sum, r) => sum + (r.spend / r.clicks || 0), 0) /
                reports.length
              : 0,
          clickThroughRate:
            reports.length > 0
              ? reports.reduce(
                  (sum, r) => sum + (r.clicks / r.impressions || 0),
                  0,
                ) / reports.length
              : 0,
          roas14d:
            reports.length > 0
              ? reports.reduce(
                  (sum, r) => sum + (r.sales14d / r.spend || 0),
                  0,
                ) / reports.length
              : 0,
          acos14d:
            reports.length > 0
              ? reports.reduce(
                  (sum, r) => sum + ((r.spend / r.sales14d) * 100 || 0),
                  0,
                ) / reports.length
              : 0,
          conversions14d: reports.reduce(
            (sum, r) => sum + (r.purchases14d / r.clicks || 0),
            0,
          ),
        };
      }
  ...
  async getAdGroupDetail(
    ...
    try {
      const reps = await this.campaignService.getAdGroupBy(
        { adGroupId, campaignId },
        ['keywords', 'ads', 'targets'],
      );
      ...
      if (reps.length) {
        ...
        if (keywords && keywords.length > 0) {
          ...
          keywordReports =
            await this.reportService.getLatestKeywordReports(keywordIds);
        }
